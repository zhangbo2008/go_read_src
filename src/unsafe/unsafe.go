...
*/
package unsafe //使用这个库包,会让代码不可移植.
...
//   - A pointer value of any type can be converted to a Pointer. //任意类型的指针可以转化为pointer
//   - A Pointer can be converted to a pointer value of any type.  // 一个指针可以转化为任意类型的指针
//   - A uintptr can be converted to a Pointer.  //uintptr可以转pointer
//   - A Pointer can be converted to a uintptr.   //uintptr是一个类型,用来存任意类型的指针.
//
// Pointer therefore allows a program to defeat the type system and read and write
// arbitrary memory. It should be used with extreme care.
//
// The following patterns involving Pointer are valid.   //下面的方式是pointer合法的使用方法,不使用下面的方式大概率是非法的使用方式. 或者将来非法的操作.
...
//
// (1) Conversion of a *T1 to Pointer to *T2.   从T1指针转T2指针
//
...
//		return *(*uint64)(unsafe.Pointer(&f)) //我们从math代码中可以看到go使用uint32或者uint64来表示bits,   这里面f是变量, &是取变量地址, *是取指针对应的值.    可以这样理解. unsafe.Pointer(&f) 是f的地址. 如果debug的话会给出一个十六进制的数. 有16个十六进制. 因为我们现在电脑都是64位系统. 所以地址用16个16进制数表示. 这时指针类型变成pointer了.我们直接*(unsafe.Pointer(&f)) 会报错. 这时我们 *uint64以下,告诉这个指针我们用uint64的指针来解析. 因为我们输入是64位的float, 所以这里转uint64, 得到的底层是bit类型64长. 我们最外层再加*就取出来这个值了. 所以函数最后我们就得到了f float64的二进制表示.
//	}
//
...
// with no pointer semantics.   uintptr本质就是一个大整数.他没有指针的语义.
...
//
//	 但是跟C不同,超出他的原始的内存分配的空间是不行的.
//		// INVALID: end points outside allocated space.
//		var s thing
//		end = unsafe.Pointer(uintptr(unsafe.Pointer(&s)) + unsafe.Sizeof(s)) //比如这个例子, 偏移量就过大.
//
//		// INVALID: end points outside allocated space.
//		b := make([]byte, n)
//		end = unsafe.Pointer(uintptr(unsafe.Pointer(&b[0])) + uintptr(n))//也是超出数组范围.
//
// Note that both conversions must appear in the same expression, with only
// the intervening arithmetic between them:  intervening 中间值. 不能存储.
//
//	// INVALID: uintptr cannot be stored in variable
//	// before conversion back to Pointer.
//	u := uintptr(p)      // uintptr不能存成一个变量.可能为了安全吧.
...
//
//	 系统调用时:
//	 如果调用uintptr,那么只能在函数里面转化, 原因就是上面的不能存uintptr这个变量.
...
//	syscall.Syscall(SYS_READ, uintptr(fd), u, uintptr(n)) //也是不等你存这个uinitptr变量.
//
// (5) Conversion of the result of reflect.Value.Pointer or reflect.Value.UnsafeAddr
// from uintptr to Pointer.     使用反射来从uintptr到pointer
//
...
//
// As in the cases above, it is invalid to store the result before the conversion: //也是不允许存中间变量.
//
//	// INVALID: uintptr cannot be stored in variable
//	// before conversion back to Pointer.
...
//
// (6) Conversion of a reflect.SliceHeader or reflect.StringHeader Data field to or from Pointer. //slice 和 string 使用反射可以得到数据的底层pointer
//
...
//
//	var s string
//	hdr := (*reflect.StringHeader)(unsafe.Pointer(&s)) // case 1 //case1,强转成stringheader指针.
//	hdr.Data = uintptr(unsafe.Pointer(p))              // case 6 (this case) //通过这行得到数据指针.
//	hdr.Len = n
//
// In this usage hdr.Data is really an alternate way to refer to the underlying
// pointer in the string header, not a uintptr variable itself.
//
// In general, reflect.SliceHeader and reflect.StringHeader should be used   //这段说只能使用 *reflect.SliceHeader and *reflect.StringHeader
...
// or struct type with elements of variable size).  //这个函数可以返回任意类型的尺寸大小. 对于数组返回成员个数, 对于结构体, 返回的是带padding的大小.
...
func Offsetof(x ArbitraryType) uintptr //返回结构体开始地址跟字段的地址的偏移值
...
// does not have variable size.
// (See the description of [Sizeof] for a definition of variable sized types.)
func Alignof(x ArbitraryType) uintptr //让变量始终是一个数值的整数倍, 这个函数就求出这个数值.
...
func SliceData(slice []ArbitraryType) *ArbitraryType //返回数组的首地址.
